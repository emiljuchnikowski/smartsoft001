...
{% callout type="warning" title="`details: true` throws on the list page" %}
The list page builds its details descriptor with a provider and with component factories, but never sets a `component`. The base list component in [`@smartsoft001/angular`](/docs/packages/angular) requires both and raises `Error('Must set details component')` as soon as `config.details` is truthy, so turning the option on breaks the screen rather than adding an inline detail panel. The examples on this page leave `details` unset, and open a record through the item page instead.
{% /callout %}
...
| `details`         | `boolean \| { cellPipe?: ICellPipe<T>; components?: { top?: any; bottom?: any } }`                                                                                                  | Inline details on the list. See the warning above before turning it on.                                                    |
