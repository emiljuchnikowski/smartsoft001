...
The spec covers the resolution in four passes. With an empty stub, the factory receives `{ httpReq, request, payloadProvider: undefined, validationProvider: undefined, userProvider: undefined }`, so the request and the Express object arrive untouched and each missing provider stays `undefined` rather than an error. With only a payload provider registered, that instance reaches the factory by identity. With a validation provider and a user provider registered together, both come through while the payload provider stays `undefined`, which proves the three lookups are independent and that an omitted `httpReq` arrives as `undefined`. And the token the factory resolves is returned by identity, so the service adds nothing to it.
