...
import { InputPhoneNumberPlPresetComponent } from './preset.component';
...
@Component({
  ...
  template: `
    <smart-input-phone-number-pl-preset
      [options]="options"
      [fieldOptions]="fieldOptions"
      [class]="cssClass"
    ></smart-input-phone-number-pl-preset>
  `,
  imports: [InputPhoneNumberPlPresetComponent],
})
...
describe('@smartsoft001/shared-angular: InputPhoneNumberPlPresetComponent', () => {
  ...
  it('should render the Polish +48 prefix addon', () => {
    const control = new UntypedFormControl('');
    host.options = buildOptions(control);
    fixture.detectChanges();
    ...
    expect(prefix?.textContent).toContain('+48');
  });

  it('should apply Preline preset classes to the input element', () => {
    const control = new UntypedFormControl('');
    host.options = buildOptions(control);
    fixture.detectChanges();
    ...
    expect(input?.className).toContain('smart:focus:ring-blue-700');
    expect(input?.className).toContain('smart:ps-12');
  });
