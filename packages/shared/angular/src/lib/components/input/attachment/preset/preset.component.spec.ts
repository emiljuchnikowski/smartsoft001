...
import { InputAttachmentPresetComponent } from './preset.component';
...
@Component({
  ...
  template: `
    <smart-input-attachment-preset
      [options]="options"
      [fieldOptions]="fieldOptions"
      [class]="cssClass"
    ></smart-input-attachment-preset>
  `,
  imports: [InputAttachmentPresetComponent],
})
...
describe('@smartsoft001/shared-angular: InputAttachmentPresetComponent', () => {
  ...
  it('should render the styled Preline drop zone with the hidden file input', () => {
    const control = new UntypedFormControl(null);
    host.options = buildOptions(control);
    fixture.detectChanges();
  ...
  it('should render TWO smart-buttons (download + delete) and file name when control.value is set', () => {
  ...
  it('should render the progress bar when loading signal is true', () => {
    ...
    const component = fixture.debugElement.query(
      By.directive(InputAttachmentPresetComponent),
    ).componentInstance as InputAttachmentPresetComponent<AttachmentModel>;
  ...
  it('should highlight the drop zone while dragging over and clear it on drop', () => {
    ...
    // jsdom has no DragEvent constructor, so use a plain Event cast to the handler type.
  ...
  it('should merge external class input into the group class attribute', () => {
    const control = new UntypedFormControl(null);
    host.options = buildOptions(control);
    ...
    const group = (fixture.nativeElement as HTMLElement).querySelector(
      'smart-input-attachment-preset > div',
    );
