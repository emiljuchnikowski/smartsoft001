...
import { InputFloatPresetComponent } from './preset.component';
...
@Component({
  ...
  template: `
    <smart-input-float-preset
      [options]="options"
      [fieldOptions]="fieldOptions"
      [class]="cssClass"
    ></smart-input-float-preset>
  `,
  imports: [InputFloatPresetComponent],
})
...
function buildOptions(control: UntypedFormControl): InputOptions<FloatModel> {
  new UntypedFormGroup({ value: control });
  return {
    control,
    fieldKey: 'value',
    model: new FloatModel(),
    treeLevel: 0,
  };
}

describe('@smartsoft001/shared-angular: InputFloatPresetComponent', () => {
