...
import { InputCurrencyPresetComponent } from './preset.component';
...
@Component({
  ...
  template: `
    <smart-input-currency-preset
      [options]="options"
      [fieldOptions]="fieldOptions"
      [class]="cssClass"
    ></smart-input-currency-preset>
  `,
  imports: [InputCurrencyPresetComponent],
})
...
describe('@smartsoft001/shared-angular: InputCurrencyPresetComponent', () => {
  ...
  it('should render the leading currency adornment with an icon', () => {
    const control = new UntypedFormControl('');
    host.options = buildOptions(control);
    fixture.detectChanges();

    const adornment = (fixture.nativeElement as HTMLElement).querySelector(
      '[data-role="currency-adornment"]',
    );

    expect(adornment).toBeTruthy();
    expect(adornment?.querySelector('svg')).toBeTruthy();
  });

  it('should pad the input start to make room for the currency adornment', () => {
    const control = new UntypedFormControl('');
    host.options = buildOptions(control);
    fixture.detectChanges();
    ...
    expect(input.className).toContain('smart:ps-11');
