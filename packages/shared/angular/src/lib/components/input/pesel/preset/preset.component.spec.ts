...
import { InputPeselPresetComponent } from './preset.component';
...
@Component({
  ...
  template: `
    <smart-input-pesel-preset
      [options]="options"
      [fieldOptions]="fieldOptions"
      [class]="cssClass"
    ></smart-input-pesel-preset>
  `,
  imports: [InputPeselPresetComponent],
})
...
function buildOptions(control: UntypedFormControl): InputOptions<PeselModel> {
  new UntypedFormGroup({ pesel: control });
  return {
    control,
    fieldKey: 'pesel',
...
describe('@smartsoft001/shared-angular: InputPeselPresetComponent', () => {
  ...
  it('should apply the Preline input look classes to the input element', () => {
    const control = new UntypedFormControl('');
    host.options = buildOptions(control);
    fixture.detectChanges();
  ...
  it('should mark control invalid for a malformed PESEL value (mask)', () => {
    const control = new UntypedFormControl('12345678901');
    host.options = buildOptions(control);
    fixture.detectChanges();
  ...
  it('should keep control valid for a correct PESEL value', () => {
    const control = new UntypedFormControl('91012008616');
    host.options = buildOptions(control);
    fixture.detectChanges();
  ...
  it('should not raise the PESEL error for an empty value', () => {
    const control = new UntypedFormControl('');
    host.options = buildOptions(control);
    fixture.detectChanges();
