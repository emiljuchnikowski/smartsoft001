...
import { InputEmailPresetComponent } from './preset.component';
...
@Component({
  ...
  template: `
    <smart-input-email-preset
      [options]="options"
      [fieldOptions]="fieldOptions"
      [class]="cssClass"
    ></smart-input-email-preset>
  `,
  imports: [InputEmailPresetComponent],
})
...
function buildOptions(control: UntypedFormControl): InputOptions<EmailModel> {
  new UntypedFormGroup({ email: control });
  return {
    control,
    fieldKey: 'email',
...
describe('@smartsoft001/shared-angular: InputEmailPresetComponent', () => {
  ...
  it('should render input element with type="email"', () => {
    const control = new UntypedFormControl('');
    host.options = buildOptions(control);
    fixture.detectChanges();
