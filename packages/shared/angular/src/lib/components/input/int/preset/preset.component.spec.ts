...
import { InputIntPresetComponent } from './preset.component';
...
@Component({
  ...
  template: `
    <smart-input-int-preset
      [options]="options"
      [fieldOptions]="fieldOptions"
      [class]="cssClass"
    ></smart-input-int-preset>
  `,
  imports: [InputIntPresetComponent],
})
...
function buildOptions(control: UntypedFormControl): InputOptions<IntModel> {
...
describe('@smartsoft001/shared-angular: InputIntPresetComponent', () => {
  ...
  it('should render input element with step="1"', () => {
    const control = new UntypedFormControl('');
    host.options = buildOptions(control);
    fixture.detectChanges();
