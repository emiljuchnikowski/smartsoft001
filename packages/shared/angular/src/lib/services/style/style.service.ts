...
export class StyleService {
  ...
  init(el: ElementRef, style?: IStyle): void {
    this._elementRef = el;
    this.set(style);
  }
  ...
  private setColor(name: string): void {
    ...
    this.setProperty(
      `--smart-color-${name}-rgb`,
      `color-${name}-rgb` as StyleType,
    );
    this.setProperty(
      `--smart-color-${name}-contrast`,
      `color-${name}-contrast` as StyleType,
    );
    this.setProperty(
      `--smart-color-${name}-contrast-rgb`,
      `color-${name}-contrast-rgb` as StyleType,
    );
    this.setProperty(
      `--smart-color-${name}-shade`,
      `color-${name}-shade` as StyleType,
    );
    this.setProperty(
      `--smart-color-${name}-tint`,
      `color-${name}-tint` as StyleType,
    );
  ...
  private setProperty(property: string, type: StyleType): void {
    if (!StyleService.style || !StyleService.style[type]) return;
