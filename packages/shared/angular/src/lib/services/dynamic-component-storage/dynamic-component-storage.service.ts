...
export class DynamicComponentStorageService {
  static get(key: string, moduleRef: NgModuleRef<any>): Type<any>[] {
    // Changed DynamicComponentType to string
    const getComponents = (k: string, m: NgModuleRef<any>): Type<any>[] => {
      // Changed DynamicComponentType to string
      if (!m) return [];
      ...
      if (fromStore) {
        ...
        m.instance &&
        m.instance.constructor &&
        (m.instance.constructor as any)['ɵmod']
      ) {
        ...
        declarations =
          (m.instance.constructor as any)['ɵmod'].declarations || [];
      } else {
        console.warn(
          'DynamicComponentStorageService: Could not retrieve declarations from moduleRef.',
        );
      }
    ...
    if (!components.length && moduleRef) {
      ...
      if (
        ...
        if (appComponentInjector) {
          ...
          if (appModuleRef && appModuleRef !== moduleRef) {
            // Avoid infinite loop if already root
    ...
    if (!components || components.length === 0) {
      console.warn(
        `DynamicComponentStorageService: No components found for key '${key}'`,
      );
    }
