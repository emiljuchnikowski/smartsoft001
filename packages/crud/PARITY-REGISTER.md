...
| GAP-07 | `StoreModule` / `NgrxSharedModule` imports removed (`CrudCoreModule`, `CrudFullModule`) | ✅ Closed   | Confirmed at runtime (Phase 5 e2e): the consuming app provides `StoreModule.forRoot` + `EffectsModule.forRoot` + `NgrxSharedModule` at the root injector; `CrudModule.forFeature` then registers the entity reducer via `NgrxStoreService.addReducer` (verified in Storybook-driven Cypress suite — `register reducer <entity>` + green read/filter/pagination/export flows). | 1     |
...
| ID        | Component                   | Missing behavior(s)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Status | Phase |
| --------- | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ----- |
| GAP-27 ✅ | `pages/list/list.component` | **DONE (Phase 3):** `<smart-page [options]="pageOptions()" [class]="config.cssClass \|\| ''">` wrapper restored, `PageComponent` imported → page **title**, **search bar**, and all **end/header buttons** (multi / filters / add / export / custom `config.buttons`) now render and are clickable. `variant: this.config.variant` threaded into `pageOptions`. Structural view-queries (`#topTpl` VCR, `#contentTpl` TemplateRef, `.dynamic-content` `DynamicContentDirective`) still resolve through the projected body (verified by AOT build + full suite; dynamic top component still mounts). | ✅     | 3     |
| GAP-28 ✅ | `pages/item/item.component` | **DONE (Phase 3):** `<smart-page [options]="pageOptions()" [class]="config.cssClass \|\| ''">` wrapper restored, `PageComponent` imported → **title** (create/update/details), mode action buttons (**add / save / cancel / edit**), **back button**, menu-button suppression now render. `variant: this.config.variant` threaded into the initial `pageOptions` signal (preserved across `initPageOptions`). `#topTpl`/`#bottomTpl`/`#contentTpl`/`.dynamic-content` view-queries resolve through the projected body.                                                                              | ✅     | 3     |
| GAP-29 ✅ | `pages/item/item.component` | **RESOLVED-BY-DESIGN (Phase 3):** with `<smart-page>`, the page body is the content container, so no Ionic content handle is needed. The stale `// @ViewChild(IonContent)` line + `//TODO: rewrite when rewriting ionic` removed. No live `content` scroll references existed in the component.                                                                                                                                                                                                                                                                                                     | ✅     | 3     |
...
- [x] GAP-07 Store/Ngrx module removal — closed in Phase 5: reducer registration confirmed at runtime in the Storybook-driven Cypress suite (host app supplies `StoreModule.forRoot`/`EffectsModule.forRoot`/`NgrxSharedModule` at root; `CrudModule.forFeature` registers the entity reducer and all read/filter/pagination/export flows pass).
...
- [x] GAP-25 multiselect header/close ✅ Phase 3 (Tailwind header — `selected: N` title + close button, OnPush) · [x] GAP-26 export popover auto-close ✅ Phase 3 (🔴 fixed — `toObservable(loaded).pipe(filter,take(1))` closes the popover after the export completes, not before/never; OnPush)
- [x] GAP-27 list `<smart-page>` ✅ Phase 3 (wrapper restored, `PageComponent` imported, `variant` + `cssClass` threaded; view-queries verified) · [x] GAP-28 item `<smart-page>` ✅ Phase 3 (wrapper restored, `variant` + `cssClass` threaded) · [x] GAP-29 item IonContent ✅ Phase 3 (resolved-by-design — `smart-page` body is the content container; stale `@ViewChild(IonContent)` comment removed, no live `content` refs)
...
- [x] **Declarative styling surface** — `CrudFullConfig.cssClass?` / `CrudFullConfig.variant?` added (tier-2 styling, OnPush-safe, additive). ✅ Phase 2. **Page-level threading delivered (Phase 3, GAP-27/28):** with the restored `<smart-page>` wrapper, `config.variant` is now threaded into the list/item `pageOptions().variant` (drives `PageComponent`'s variant resolution) and `config.cssClass` is bound via `[class]="config.cssClass || ''"` on `<smart-page>` (forwarded to the variant component's `cssClass` input). The previously-deferred deep threading is thus **partially delivered at the page level**; deeper per-field threading into `IListOptions`/cell-level styling remains a future enhancement.
